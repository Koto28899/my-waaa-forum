...
func (*Follow) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case follow.FieldID, follow.FieldFromRoleID, follow.FieldToRoleID:
			values[i] = new(sql.NullInt64)
		case follow.FieldCreatedAt, follow.FieldUpdatedAt, follow.FieldDeletedAt:
...
func (f *Follow) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", f.FromRoleID))
	builder.WriteString(", ")
	builder.WriteString("to_role_id=")
	builder.WriteString(fmt.Sprintf("%v", f.ToRoleID))
	builder.WriteByte(')')
