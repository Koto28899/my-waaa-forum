...
func (sc *SessionCreate) SetRoleID(i int64) *SessionCreate {
...
func (sc *SessionCreate) defaults() {
	...
	if _, ok := sc.mutation.IsBlocked(); !ok {
		v := session.DefaultIsBlocked
		sc.mutation.SetIsBlocked(v)
	}
}
...
func (sc *SessionCreate) check() error {
	if _, ok := sc.mutation.RoleID(); !ok {
		return &ValidationError{Name: "role_id", err: errors.New(`ent: missing required field "Session.role_id"`)}
	}
	if v, ok := sc.mutation.RoleID(); ok {
		if err := session.RoleIDValidator(v); err != nil {
			return &ValidationError{Name: "role_id", err: fmt.Errorf(`ent: validator failed for field "Session.role_id": %w`, err)}
		}
	}
	if _, ok := sc.mutation.CreatedAt(); !ok {
	...
	if v, ok := sc.mutation.ClientIP(); ok {
		if err := session.ClientIPValidator(v); err != nil {
			return &ValidationError{Name: "client_ip", err: fmt.Errorf(`ent: validator failed for field "Session.client_ip": %w`, err)}
		}
	}
	if _, ok := sc.mutation.IsBlocked(); !ok {
		return &ValidationError{Name: "is_blocked", err: errors.New(`ent: missing required field "Session.is_blocked"`)}
	}
	return nil
}
...
func (sc *SessionCreate) createSpec() (*Session, *sqlgraph.CreateSpec) {
	...
	if value, ok := sc.mutation.RoleID(); ok {
		_spec.SetField(session.FieldRoleID, field.TypeInt64, value)
		_node.RoleID = value
	}
