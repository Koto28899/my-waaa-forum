...
type Post struct {
	...
	// ToSectionID holds the value of the "to_section_id" field.
	ToSectionID int64 `json:"to_section_id,omitempty"`
	...
	// IsHighlight holds the value of the "is_highlight" field.
...
func (*Post) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case post.FieldIsTop, post.FieldIsHighlight:
			values[i] = new(sql.NullBool)
		case post.FieldID, post.FieldFromRoleID, post.FieldToSectionID, post.FieldLikesCount:
	...
	for i := range columns {
		...
		case post.FieldFromRoleID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				po.FromRoleID = value.Int64
			}
		case post.FieldToSectionID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field to_section_id", values[i])
			} else if value.Valid {
				po.ToSectionID = value.Int64
			}
		case post.FieldLikesCount:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field likes_count", values[i])
			} else if value.Valid {
				po.LikesCount = value.Int64
			}
		...
		case post.FieldIsHighlight:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field is_highlight", values[i])
			} else if value.Valid {
				po.IsHighlight = value.Bool
			}
...
func (po *Post) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", po.FromRoleID))
	builder.WriteString(", ")
	builder.WriteString("to_section_id=")
	builder.WriteString(fmt.Sprintf("%v", po.ToSectionID))
	builder.WriteString(", ")
	builder.WriteString("likes_count=")
	builder.WriteString(fmt.Sprintf("%v", po.LikesCount))
	builder.WriteString(", ")
	builder.WriteString("is_top=")
	builder.WriteString(fmt.Sprintf("%v", po.IsTop))
	builder.WriteString(", ")
	builder.WriteString("is_highlight=")
	builder.WriteString(fmt.Sprintf("%v", po.IsHighlight))
	builder.WriteString(", ")
